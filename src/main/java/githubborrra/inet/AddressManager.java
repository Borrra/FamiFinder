			...
			try {
				...
				if ( stuff.size() == 2 && a==0) { // для случая поиска обоих файлов, а в скрытом 2 строки
					...
					if (findFile1.exists() && a23[0].contains(textFileName) && findFile2.exists() && a23[1]
						...
						//System.out.println("1случай_Было 2 файла, один Фотки, другой ТекстФайл");

						//ServiceMethods.windowShow("1случай_Было 2 файла, один Фотки, другой ТекстФайл");
						...
						//System.out.println("2случай_Было 2 файла, один Фотки, другой ТекстФайл");
						
						//ServiceMethods.windowShow("2случай_Было 2 файла, один Фотки, другой ТекстФайл");
					...
					/* сюда попадаем, если в скрытый файл записали один адрес, но система считывает их как 2,
					 * поэтому из этих 2-х адресов выбираем один, содержащий имя либо Текст файла либо папки с фотками */
						...
						if ( findFile1.exists() ) {
							...
							if ( a23[0].contains(textFileName) ) {
								...
								//ServiceMethods.windowShow("3случай_Был один адрес он оказался Текстовым");
								...
								//ServiceMethods.windowShow("4случай_Был один адрес он оказался Фотками");
							...
							if (  a23[1].contains(textFileName) )  {
								...
								//ServiceMethods.windowShow("5случай_Был один адрес он оказался Текстовым");
								...
								AddressManager manah = new AddressManager (1, a23[1]); // конструктор № 3 - здесь была ошибка!

								//ServiceMethods.windowShow("6случай_Был один адрес он оказался Фотками");
	...
	private void writingFilesToHiddingFile(AddressManager manag) {
		...
		try {
			...
			if (Files.notExists(hiddenFile)) {
				...
				ServiceMethods.windowShow("ТекстФайл: " + manag.fileAddress);
				ServiceMethods.windowShow("ФотоПапка: " + manag.photoFolderAddress);
